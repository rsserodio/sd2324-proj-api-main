...
class DiscoveryImpl implements Discovery {
	...
	private Map<String, ArrayList<URI>> knownURIs = new HashMap<String, ArrayList<URI>>();
	...
	public URI[] knownUrisOf(String serviceName, int minEntries) {
		ArrayList<URI> auxURIs = null;
		
		while(auxURIs == null) {
			auxURIs = knownURIs.get(serviceName);
		}
		
		while(auxURIs.size() < minEntries) {
			auxURIs = knownURIs.get(serviceName);
		}
		
		URI[] arrayURI = new URI[auxURIs.size()];
        arrayURI = auxURIs.toArray(arrayURI);
		return arrayURI;
	}
				...
				for (;;) {
					try {
						...
						if (parts.length == 2) {
							...
							ArrayList<URI> auxURIs = knownURIs.get(serviceName);
							URI auxURI = uri;
							if(auxURIs == null) {
								auxURIs = new ArrayList<URI>();
							}
							auxURIs.add(auxURI);
							knownURIs.put(serviceName, auxURIs);	
